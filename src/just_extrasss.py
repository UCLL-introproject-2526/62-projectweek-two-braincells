...
class HomeScene:
    ...
    def draw(self,surf):
        ...
        badge = FONT_SMALL.render(f"Player: {self.app.username}", True, (230,230,240))
        badge_bg = pygame.Surface((badge.get_width()+20, badge.get_height()+10), pygame.SRCALPHA)
        badge_bg.fill((0,0,0,120))
        surf.blit(badge_bg, (W//2 - badge_bg.get_width()//2, 140))
        surf.blit(badge, (W//2 - badge.get_width()//2, 145))
...
class TextInput:
    ...
    def draw(self, surf):
        # Background with a soft vertical gradient (top to bottom)
        grad_height = self.rect.height
        for i in range(grad_height):
            # interpolate between two colors
            c1 = pygame.Color(60, 60, 70)    # top color
            c2 = pygame.Color(40, 40, 48)    # bottom color
        ...
        # Draw glowing outer border if active (soft glow)
        if self.active:
