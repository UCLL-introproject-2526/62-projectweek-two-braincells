...
try:
    tree_left, _ = load_image("tree_img/tree_left_final_clean2.png", convert_alpha=True)
    tree_middle, _ = load_image("tree_img/tree_middle_final_clean2.png", convert_alpha=True)
    tree_right, _ = load_image("tree_img/tree_right_final_clean2.png", convert_alpha=True)
...
tongue_paths = [
    "tongues_split_pngs",
    os.path.join(os.path.expanduser("~"), "Downloads", "tongues_split_pngs")
]
    ...
    for anim_type in ["standing", "walk", "jump"]:
        ...
        for i in range(1, frame_count + 1):
            for direction in ["left", "right"]:
                ...
                path = f"frogs_split/{anim_type}_{direction}_f{i}.png"
                if os.path.exists(path):
                    try:
                        frame = pygame.image.load(path).convert_alpha()
...
if os.path.exists("pixel_font"):
    for char in "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        path = os.path.join("pixel_font", f"{char}.png")
        if os.path.exists(path):
            try:
...
while running:
    ...
    if (SWAMP_START_X <= character_center_x <= SWAMP_START_X + SWAMP_WIDTH and 
        character_bottom >= GROUND_Y and not on_platform):
        reset_game()
